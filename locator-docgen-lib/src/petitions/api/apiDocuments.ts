...
function apiDocumentsFunction(options?: DocumentApiOptions) {
  const config = getConfig();

  async function generateDocument(document: EnhancedDocumentData, templateName: string) {
    try {
      const request: DocumentGenerationRequest = {
        type: "docx",
        template_fields: JSON.stringify(document),
...
// Именованный экспорт
export const apiDocuments = apiDocumentsFunction;

// Экспорт по умолчанию для совместимости
export default apiDocumentsFunction;